...
export default function PostPage({ userInfo }: { userInfo: userInfo }) {
  ...
  function fetchComments() {
    ...
    fetch(`${"https://react-nest-blog-platform-production.up.railway.app/"}comments?page=${commentsPage}&postId=${postId}`, {
  ...
  function handleSave() {
    ...
    const token = window.sessionStorage.getItem("access_token");
    fetch(`${"https://react-nest-blog-platform-production.up.railway.app/"}posts/save/${postId}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => {
        if (!res.ok) {
  ...
  function fetchPost() {
    const token = window.sessionStorage.getItem("access_token");
    fetch(`${"https://react-nest-blog-platform-production.up.railway.app/"}posts/${postId}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then(async (res) => {
  ...
  function handleSubmit() {
    ...
    fetch(`${"https://react-nest-blog-platform-production.up.railway.app/"}comments`, {
      method: "POST",
      headers: {
  ...
  function handleLike() {
    ...
    const token = window.sessionStorage.getItem("access_token");
    fetch(`${"https://react-nest-blog-platform-production.up.railway.app/"}posts/like/${postId}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => {
        if (!res.ok) {
        ...
        <Box display="flex" alignItems="center" justifyContent="start" sx={{ width: "60%" }} gap={3} mb={2}>
          ...
          >
            {post.user?.profilePictureId ? (
              <Box
                component="img"
                src={`${"https://react-nest-blog-platform-production.up.railway.app/"}image/${post.user?.profilePictureId}`}
...
function CommentCard({
  ...
  function handleAnswerFetch() {
    const token = window.sessionStorage.getItem("access_token");
    fetch(`${"https://react-nest-blog-platform-production.up.railway.app/"}comments/answers?commentId=${commentData._id}&page=${answerPage}`, {
      headers: { Authorization: `Bearer ${token}` },
    }).then((res) => {
      if (!res.ok) return;
      res.json().then((result) => {
  ...
  function handleLike() {
    ...
    fetch(`${"https://react-nest-blog-platform-production.up.railway.app/"}comments/${commentData._id}/like`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => {
        if (!res.ok) {
      ...
      <Box display="flex" alignItems="center" gap={1}>
        ...
        >
          {commentData.user?.profilePictureId ? (
            <Box
              component="img"
              src={`${"https://react-nest-blog-platform-production.up.railway.app/"}image/${commentData.user?.profilePictureId}`}
...
/*
  function handleLike() {
    ...
    fetch(`${"https://react-nest-blog-platform-production.up.railway.app/"}comments/${reply._id}/like`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => {
        if (!res.ok) {
      ...
      <Box display="flex" alignItems="center" gap={1}>
        ...
        >
          {reply.user?.profilePictureId ? (
            <Box
              component="img"
              src={`${"https://react-nest-blog-platform-production.up.railway.app/"}image/${reply.user?.profilePictureId}`}
              sx={{ width: "100%", height: "100%", borderRadius: "50%" }}
            />
          ) : (
...
function ReportPostForm({
  ...
  function handleSubmit() {
    ...
    const token = window.sessionStorage.getItem("access_token");
    fetch(`${"https://react-nest-blog-platform-production.up.railway.app/"}posts/report/${postId}`, {
      method: "POST",
      headers: { Authorization: `Bearer  ${token}` },
    })
      .then(async (res) => {
