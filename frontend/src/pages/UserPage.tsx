...
export default function UserPage({ currentUserName }: { currentUserName: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`${"https://react-nest-blog-platform-production.up.railway.app/"}user/profile/${userName}`, {
  ...
  function handleFollow() {
    ...
    const token = window.sessionStorage.getItem("access_token")
    fetch(`${"https://react-nest-blog-platform-production.up.railway.app/"}user/follow/${userName}`, { headers: { Authorization: `Bearer ${token}` } }).then((res) => {
      if (!res.ok) {
    ...
    <AppLayout>
      ...
      >
        ...
        >
          ...
          >
            <Avatar
              src={userInfo?.profilePictureId && `${"https://react-nest-blog-platform-production.up.railway.app/"}image/${userInfo?.profilePictureId}`}
              sx={{
                width: 120,
                height: 120,
                mb: 2,
